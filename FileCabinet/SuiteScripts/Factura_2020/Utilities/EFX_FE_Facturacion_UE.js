...
define(['N/record', 'N/search', 'N/runtime', './EFX_FE_Importe_en_Letra_ST', 'N/record', 'N/currency', 'N/config', './moment.js', 'N/query', 'N/file', 'N/format', 'N/ui/serverWidget', 'N/ui/message', 'N/url', 'N/https'],
    ...
    function (record, search, runtime, mod_monto_letra, modRecord, modcurrency, config, moment, query, file, format, serverWidget, message, url, https) {
        ...
        function beforeSubmit(context) {
            var newRec = context.newRecord;
            var recType = newRec.type;
            //! Script personalizado de qima
            log.debug('test1', 'NYCE - Set File Online');
            if (context.type !== context.UserEventType.CREATE && context.type !== context.UserEventType.EDIT) {
                log.debug('test2', 'NYCE - Set File Online');

                return;
            }
            var subsidiaryId = newRec.getValue({ 'fieldId': 'subsidiary' });
            if (subsidiaryId != '93' && subsidiaryId != '97') return;

            // Get the number of lines on the item sublist
            // var lineCount = newRec.getLineCount({ sublistId: 'item' });

            // NEW
            var notasNYCE1 = newRec.getSublistValue({
                sublistId: 'item',
                fieldId: 'custcol_tkio_notas_fact',
                line: 0
            });
            var notasNYCE2 = newRec.getSublistValue({
                sublistId: 'item',
                fieldId: 'custcol_tkio_notas_fact_2',
                line: 0
            });
            var notasNYCE3 = newRec.getSublistValue({
                sublistId: 'item',
                fieldId: 'custcol_tkio_notas_fact_3',
                line: 0
            });
            var concatenatedValues = notasNYCE1 + ' ' + notasNYCE2 + ' ' + notasNYCE3;
            log.debug('concatenatedValues', concatenatedValues);
            newRec.setValue({
                fieldId: 'custbody_notas_nyce_concat',
                value: concatenatedValues
            });
            //! termina bloque de codigo personalizado de qima
        ...
        function afterSubmit(context) {
            ...
            if (context.type != context.UserEventType.DELETE) {
                ...
                // ! Script personalizado de qima
                try {
                    log.audit({ title: 'LOG', details: 'NYCE - Set File Online' });
                    log.debug({ 'title': 'afterSubmit:context.type', 'details': context.type });
                    // if (context.type != 'create' && context.type != 'edit' && context.type != 'csv') return;
                    if (context.type != 'create' && context.type != 'edit' && context.type != 'xedit' && context.type != 'csv') return;

                    // var record = context.newRecord;

                    // Check if valid Subsidiary

                    if (context.type == "xedit") {
                        var data_transaction = search.lookupFields({
                            type: recType,
                            id: recID,
                            columns: ['subsidiary', 'custbody_edoc_generated_pdf', 'custbody_psg_ei_certified_edoc']
                        });
                        log.audit({title: 'data_transaction', details: data_transaction});
                    }
                    var subsidiaryId = data_transaction.subsidiary[0].value;
                    log.audit({title: 'id subsidiary', details: subsidiaryId});
                    if (subsidiaryId != '93') return;
                    log.audit({title: 'afterSubmit:validation ~ 164', details: 'pasa la validaion'});

                    // Fields to update
                    var updateFields = {};

                    // Process Generated PDF field
                    // log.debug({'title': 'afterSubmit:generatedPdfId', 'details': generatedPdfId});
                    var generatedPdfId = data_transaction.custbody_edoc_generated_pdf[0].value;
                    log.audit({title: 'pdf ID', details: generatedPdfId});
                    if (generatedPdfId) {
                        var generatedPdfUrl = setFileOnline(generatedPdfId);
                        updateFields.custbody_nyce_generatedpdfurl = formatUrl(generatedPdfUrl);
                    }

                    // Process Certified E-Document field
                    // log.debug({'title': 'afterSubmit:certifiedEDocumentId', 'details': certifiedEDocumentId});
                    var certifiedEDocumentId = data_transaction.custbody_edoc_generated_pdf[0].value;
                    log.audit({title: 'xml ID', details: certifiedEDocumentId});
                    if (certifiedEDocumentId) {
                        var certifiedEDocumentUrl = setFileOnline(certifiedEDocumentId);
                        updateFields.custbody_nyce_certifiededocumenturl = formatUrl(certifiedEDocumentUrl);
                    }

                    // Save processed data
                    log.debug({ 'title': 'afterSubmit:updateFields', 'details': updateFields });
                    // log.debug({'title': 'afterSubmit:newRec.id', 'details': newRec.id});
                    // log.debug({'title': 'afterSubmit:newRec.type', 'details': newRec.type});
                    record.submitFields({
                        'type': recType,
                        'id': recID,
                        'values': updateFields,
                    ...
                    log.debug({ 'title': 'afterSubmit:UPDATED', 'details': recType + ':' + recID });
                } catch (error) {
                    log.audit({ 'title': 'afterSubmit:error', 'details': error });
                }
                // ! Finaliza bloque de codigo personalizado de qima

                log.audit({ title: 'tipo del registro a cargar: ', details: recType });
                log.audit({ title: 'ID del registro a cargar: ', details: recID });
        ...
        // ! funcion custom para el script de qima
        function setFileOnline(fileId) {
            try {
                log.audit({title: 'id del archivo a cargar: ', details: fileId});
                var archivo = file.load({ 'id': fileId });
                archivo.isOnline = true;

                var fileUrl = archivo.url;

                fileId = archivo.save()

                log.debug({ 'title': 'setFileOnline:fileId', 'details': fileId });
                log.debug({ 'title': 'setFileOnline:fileUrl', 'details': fileUrl });

                return fileUrl;
            } catch (error) {
                log.audit({ 'title': 'setFileOnline:error', 'details': error });
            }
        }

        // ! funcion custom para el script de qima
        function formatUrl(url) {
            var domain = 'https://4552646.app.netsuite.com';

            url = domain + url + '&_xd=T';
        ...
        function buscarDireccionesCP(id_cliente, id_subsidiaria, obj_direccion, SUBSIDIARIES, creadodeff, creadodeffid, tipoTransaccionff, usocfdiIFF) {
            ...
            if (tipoTransaccionff == 'transferorder') {
                ...
                } catch (ubicacionOrigelog) {
                    log.audit({ title: 'ubicacionOrigelog', details: JSON.stringify(ubicacionOrigelog) });
                }
